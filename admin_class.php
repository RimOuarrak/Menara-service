...
Class Action {
	...
	function save_org(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO org set $data");
		}else{
			$save = $this->db->query("UPDATE org set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function save_link(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO docs set $data");
		}else{
			$save = $this->db->query("UPDATE docs set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_link(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM docs where id = $id");
		if($delete){
			return 1;
		}
	}
